...
*/
       ...
       // System.out.println();
        ...
        for (int i = 0; i < array.length; i++) {
            for (int j = 0; j < array.length - 1; j++) { //(длина массива минус 1, потому что последний элемент переставлять не нужно
                if (array[j] == 0) { //если j-ый элемент равен нулю
                    int temp = array[j]; //создаем временную переменную и помещаем туда его значение
                    array[j] = array[j + 1];//перестановка j-го элемента с нулем вправо на 1 индекс
                    array[j + 1] = temp;
                }
            }
        }
